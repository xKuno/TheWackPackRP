    ...
    methods: {
        changePage(page) {
            ...
            switch (page) {
                ...
                case "Warrants": {
                    $.post("http://wp_doj/getWarrants", JSON.stringify({}));
                    $("#warrants").addClass("nav-active");
                    break;
                }
                case "Bolos": {
                    $.post("http://wp_doj/getBolos", JSON.stringify({}));
        ...
        closeDOJ() {
            $.post("http://wp_doj/close", JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {
                this.offender_results.query = this.offender_search;

                $.post("http://wp_doj/performOffenderSearch", JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (let key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    $.post("http://wp_doj/viewOffender", JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post("http://wp_doj/saveOffenderChanges", JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post("http://wp_doj/performReportSearch", JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post("http://wp_doj/submitNewReport", JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post("http://wp_doj/getOffender", JSON.stringify({
                char_id: id
            }));
        },
        ToggleReportEdit() {
        ...
        DeleteSelectedReport() {
            $.post("http://wp_doj/deleteReport", JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post("http://wp_doj/saveReportChanges", JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post("http://wp_doj/vehicleSearch", JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post("http://wp_doj/getVehicle", JSON.stringify({
                vehicle: result
            }));
        },
        WarrantReportSearch() {
        ...
        SubmitNewWarrant() {
            ...
            $.post("http://wp_doj/submitNewWarrant", JSON.stringify({
        ...
        SubmitNewBolo() {
            ...
            $.post("http://wp_doj/submitNewBolo", JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post("http://wp_doj/deleteWarrant", JSON.stringify({
        ...
        DeleteSelectedBolo() {
            $.post("http://wp_doj/deleteBolo", JSON.stringify({
        ...
        OpenReportById(id) {
            $.post("http://wp_doj/getReport", JSON.stringify({
                id: id
            }));
        },
        ...
        SentencePlayer() {
            $.post("http://wp_doj/sentencePlayer", JSON.stringify({
                id: this.report_selected.id,
    ...
    if (document.readyState === "complete") {
        window.addEventListener("message", function(event) {
            if (event.data.type == "enable") {
                ...
                dojApp.offender_results.results = event.data.matches;
            } else if (event.data.type == "returnedOffenderDetails") {
                dojApp.offender_selected = event.data.details;
                dojApp.offender_results.results = false;
                dojApp.offender_results.query = "";
                dojApp.offender_changes.notes = dojApp.offender_selected.notes;
                dojApp.offender_changes.mugshot_url = dojApp.offender_selected.mugshot_url;
                dojApp.offender_changes.licenses = dojApp.offender_selected.licenses;
                dojApp.offender_changes.licenses_removed = [];
                dojApp.offender_changes.convictions = dojApp.offender_selected.convictions;
                dojApp.offender_changes.convictions_removed = [];
                dojApp.offender_search = dojApp.offender_selected.firstname + " " + dojApp.offender_selected.lastname;
                dojApp.changePage("Search Offenders");

                dojApp.recent_searches.person.unshift(event.data.details);
                if (dojApp.recent_searches.person.length > 3) {
                    Vue.delete(dojApp.recent_searches.person, 3)
                }
            } else if (event.data.type == "offensesAndOfficerLoaded") {
                dojApp.offenses = event.data.offenses;
                dojApp.officer.name = event.data.name;
            } else if (event.data.type == "returnedReportMatches") {
                dojApp.report_results.results = event.data.matches;
            } else if (event.data.type == "returnedVehicleMatches") {
                dojApp.vehicle_results.results = event.data.matches;
                dojApp.vehicle_selected = {
                    plate: null,
                    type: null,
                    owner: null,
                    owner_id: null,
                ...
                dojApp.vehicle_selected = event.data.details;
                dojApp.vehicle_search = dojApp.vehicle_selected.plate;

                dojApp.recent_searches.vehicle.unshift(event.data.details);
                if (dojApp.recent_searches.vehicle.length > 3) {
                    Vue.delete(dojApp.recent_searches.vehicle, 3)
                }
            } else if (event.data.type == "returnedVehicleMatchesInFront") {
                dojApp.vehicle_results.results = event.data.matches;
                dojApp.vehicle_results.query = event.data.plate;
                dojApp.vehicle_search = event.data.plate;
                dojApp.vehicle_selected = {
                    plate: null,
                    type: null,
                    owner: null,
                ...
                dojApp.changePage("Search Vehicles");
            } else if (event.data.type == "returnedWarrants") {
                dojApp.warrants = event.data.warrants;
            } else if (event.data.type == "returnedBolos") {
                dojApp.bolos = event.data.bolos;
            } else if (event.data.type == "completedWarrantAction") {
                dojApp.changePage("Warrants");
            } else if (event.data.type == "completedBoloAction") {
                dojApp.changePage("Bolos");
            } else if (event.data.type == "returnedReportDetails") {
                dojApp.changePage("Search Reports");
                dojApp.report_selected = event.data.details;
            } else if (event.data.type == "recentReportsAndWarrantsLoaded") {
                dojApp.homepage.reports = event.data.reports;
                dojApp.homepage.warrants = event.data.warrants;
                dojApp.homepage.bolos = event.data.bolos;
                dojApp.officer.name = event.data.officer;
                dojApp.officer.job = event.data.job;
            };
        });
    };
};
...
document.onkeydown = function (data) {
    if (data.which == 27 || data.which == 112) { // ESC or F1
        ...
        // Stop enter key from crashing DOJ in an input?
...
function WarrantTimer() {
    setInterval(function() {
        for (let key in dojApp.warrants) {
            const warrant = dojApp.warrants[key];
            ...
            if (t >= 0) {
                ...
                Vue.delete(dojApp.warrants, key)
            }
        }
    }, 1000);
}
...
function BoloTimer() {
    setInterval(function() {
        for (let key in dojApp.bolos) {
            const bolo = dojApp.bolos[key];
