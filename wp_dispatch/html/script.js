...
$(document).keyup(function (e) {
  if (e.keyCode === 27) {
    ...
    $.post('https://wp_dispatch/close', JSON.stringify({}));
...
function forwardCall(num) {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == num) {
      $.post('https://wp_dispatch/forwardCall', JSON.stringify({
...
function copyNumber() {
  ...
  $.post('https://wp_dispatch/copynumber', JSON.stringify({}));
}

function toggleOffduty() {

  $.post('https://wp_dispatch/toggleoffduty', JSON.stringify({}));

}

function togglecallblips() {
  $.post('https://wp_dispatch/togglecallblips', JSON.stringify({}));
}

function toggleUnitBlips() {
  $.post('https://wp_dispatch/toggleunitblips', JSON.stringify({}));
}

function toggleAlerts() {

  $.post('https://wp_dispatch/togglealerts', JSON.stringify({}));

}

function changeToBusy() {
  $.post('https://wp_dispatch/updatestatus', JSON.stringify({
    id: id,
    status: 'Busy'
  }));
}
...
function dismissCall() {
  ...
  acceptedCall = '';
  $.post('https://wp_dispatch/dismissCall', JSON.stringify({
    id: currentElement.id
  }));
...
function removeCall() {
  ...
  $.post('https://wp_dispatch/removeCall', JSON.stringify({
    id: currentElement.id
  }));
...
function acceptCall() {
  ...
  $.post('https://wp_dispatch/acceptCall', JSON.stringify({
    id: currentElement.id,
  ...
  $.post('https://wp_dispatch/sendnotice', JSON.stringify({
...
function requestBackup() {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == col) {
      $.post('https://wp_dispatch/reqbackup', JSON.stringify({
        job: key,
