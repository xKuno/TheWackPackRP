...
local weapons = {
    ...
    [tostring(GetHashKey('WEAPON_REVOLVER_MK2'))] = { 
        ['scope'] = GetHashKey('COMPONENT_AT_SIGHTS'),
		['flashlight'] = GetHashKey('COMPONENT_AT_PI_FLSH')
    },
    ...
    [tostring(GetHashKey('WEAPON_COMBATPDW'))] = { 
        ...
        ['scope'] = GetHashKey('COMPONENT_AT_AR_AFGRIP'),
...
AddEventHandler('DP_Inventory:useAttach', function(attach)
    ...
    if currentWeapon ~= nil then
        ...
        elseif string.find(attach, 'skin') then
            ...
            exports['mythic_notify']:SendAlert('inform', 'This attachment is not compatible or is already equipped!', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' }) -- RED
        end
    else
        exports['mythic_notify']:SendAlert('inform', 'No weapon selected.!', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' }) -- RED
    end
end)
...
function RemoveWeapon(weapon)
    ...
    ESX.TriggerServerCallback('DP_Inventory:doesWeaponHas', function(hasWeaponId)
        ...
        if ESX.GetPlayerData().job ~= nil and ESX.GetPlayerData().job.name == 'police' then
