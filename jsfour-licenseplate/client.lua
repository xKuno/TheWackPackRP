  ...
  ESX.UI.Menu.Open(
	...
	function(data, menu)
      ...
      if plate ~= nil then
        ...
        if distance ~= -1 and distance <= 3.0 then
          ...
          ESX.TriggerServerCallback('jsfour-licenseplate:update', function( cb )
            if cb == 'found' then
              ...
              local upplate = string.upper(plate)
              TriggerServerEvent('RufiCarkey:DeletePlate', oldplate)
              TriggerServerEvent('RufiCarkey:RegisterPlate', upplate, vehicleModel)
              plate = nil
            elseif cb == 'error' then
              ESX.ShowNotification('You couldn\'t apply the license plate')
            end
          end, oldplate, plate)
        else
        ...
        ESX.ShowNotification('Can\'t apply empty plate')
      end
		end
	end,
	function(data, menu)
...
-- Menu2
RegisterNetEvent('jsfour-fakelicenseplate')
AddEventHandler('jsfour-fakelicenseplate', function()
  ESX.UI.Menu.CloseAll()
  ESX.UI.Menu.Open(
	'default', GetCurrentResourceName(), 'licenseplate_menu',
	{
	...
	function(data, menu)
      ...
      if plate ~= nil then
        ...
        if distance ~= -1 and distance <= 3.0 then
          ...
          ESX.TriggerServerCallback('jsfour-fakelicenseplate:update', function( cb )
            if cb == 'found' then
              ESX.UI.Menu.CloseAll()
              local vehicleModel = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))
              SetVehicleNumberPlateText(vehicle, plate)
              local upplate = string.upper(plate)
              TriggerEvent('RufiCarkey:TempKey', upplate)
              plate = nil
            elseif cb == 'error' then
              ESX.ShowNotification('You couldn\'t apply the license plate')
            end
