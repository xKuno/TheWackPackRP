						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to', {
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									...
									exports['mythic_notify']:SendAlert('inform', 'You cannot give anything to someone in a vehicle.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
								end
							else
				...
				if IsPedOnFoot(closestPed) and not IsPedFalling(closestPed) then
					if closestPlayer ~= -1 and closestDistance < 3.0 then
						if pedAmmo > 0 then
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										...
										exports['mythic_notify']:SendAlert('inform', 'You do not have enough ammo.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
									end
								else
									exports['mythic_notify']:SendAlert('inform', 'Invalid amount.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
								end
							...
							exports['mythic_notify']:SendAlert('inform', 'You do not have enough.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
						end
					else
						exports['mythic_notify']:SendAlert('inform', 'No players nearby.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
					end
