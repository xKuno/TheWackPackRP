    ...
    methods: {
        changePage(page) {
            ...
            switch (page) {
                ...
                case "Warrants": {
                    $.post("http://wp_emsmdt/getWarrants", JSON.stringify({}));
                    $("#warrants").addClass("nav-active");
                    break;
                }
                case "Bolos": {
                    $.post("http://wp_emsmdt/getBolos", JSON.stringify({}));
        ...
        closeems() {
            $.post("http://wp_emsmdt/close", JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {
                this.offender_results.query = this.offender_search;

                $.post("http://wp_emsmdt/performOffenderSearch", JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (let key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    $.post("http://wp_emsmdt/viewOffender", JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post("http://wp_emsmdt/saveOffenderChanges", JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post("http://wp_emsmdt/performReportSearch", JSON.stringify({
        ...
        AddCharge(id) {
            for (let key in this.offenses) {
                if (id == this.offenses[key].id) {
                    ...
                    this.report_new.recommended_fine = Math.min(this.report_new.recommended_fine + this.offenses[key].amount, 10000)
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post("http://wp_emsmdt/submitNewReport", JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post("http://wp_emsmdt/getOffender", JSON.stringify({
                char_id: id
            }));
        },
        ToggleReportEdit() {
        ...
        DeleteSelectedReport() {
            $.post("http://wp_emsmdt/deleteReport", JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post("http://wp_emsmdt/saveReportChanges", JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post("http://wp_emsmdt/vehicleSearch", JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post("http://wp_emsmdt/getVehicle", JSON.stringify({
                vehicle: result
            }));
        },
        WarrantReportSearch() {
        ...
        SubmitNewWarrant() {
            ...
            $.post("http://wp_emsmdt/submitNewWarrant", JSON.stringify({
        ...
        SubmitNewBolo() {
            ...
            $.post("http://wp_emsmdt/submitNewBolo", JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post("http://wp_emsmdt/deleteWarrant", JSON.stringify({
        ...
        DeleteSelectedBolo() {
            $.post("http://wp_emsmdt/deleteBolo", JSON.stringify({
        ...
        OpenReportById(id) {
            $.post("http://wp_emsmdt/getReport", JSON.stringify({
                id: id
            }));
        },
        ...
        SentencePlayer() {
            $.post("http://wp_emsmdt/sentencePlayer", JSON.stringify({
                id: this.report_selected.id,
    ...
    if (document.readyState === "complete") {
        window.addEventListener("message", function(event) {
            if (event.data.type == "enable") {
                ...
                emsApp.offender_results.results = event.data.matches;
            } else if (event.data.type == "returnedOffenderDetails") {
                emsApp.offender_selected = event.data.details;
                emsApp.offender_results.results = false;
                emsApp.offender_results.query = "";
                emsApp.offender_changes.notes = emsApp.offender_selected.notes;
                emsApp.offender_changes.mugshot_url = emsApp.offender_selected.mugshot_url;
                emsApp.offender_changes.licenses = emsApp.offender_selected.licenses;
                emsApp.offender_changes.licenses_removed = [];
                emsApp.offender_changes.convictions = emsApp.offender_selected.convictions;
                emsApp.offender_changes.convictions_removed = [];
                emsApp.offender_search = emsApp.offender_selected.firstname + " " + emsApp.offender_selected.lastname;
                emsApp.changePage("Search Offenders");

                emsApp.recent_searches.person.unshift(event.data.details);
                if (emsApp.recent_searches.person.length > 3) {
                    Vue.delete(emsApp.recent_searches.person, 3)
                }
            } else if (event.data.type == "offensesAndOfficerLoaded") {
                emsApp.offenses = event.data.offenses;
                emsApp.officer.name = event.data.name;
            } else if (event.data.type == "returnedReportMatches") {
                emsApp.report_results.results = event.data.matches;
            } else if (event.data.type == "returnedVehicleMatches") {
                emsApp.vehicle_results.results = event.data.matches;
                emsApp.vehicle_selected = {
                    plate: null,
                    type: null,
                    owner: null,
                    owner_id: null,
                ...
                emsApp.vehicle_selected = event.data.details;
                emsApp.vehicle_search = emsApp.vehicle_selected.plate;

                emsApp.recent_searches.vehicle.unshift(event.data.details);
                if (emsApp.recent_searches.vehicle.length > 3) {
                    Vue.delete(emsApp.recent_searches.vehicle, 3)
                }
            } else if (event.data.type == "returnedVehicleMatchesInFront") {
                emsApp.vehicle_results.results = event.data.matches;
                emsApp.vehicle_results.query = event.data.plate;
                emsApp.vehicle_search = event.data.plate;
                emsApp.vehicle_selected = {
                    plate: null,
                    type: null,
                    owner: null,
                ...
                emsApp.changePage("Search Vehicles");
            } else if (event.data.type == "returnedWarrants") {
                emsApp.warrants = event.data.warrants;
            } else if (event.data.type == "returnedBolos") {
                emsApp.bolos = event.data.bolos;
            } else if (event.data.type == "completedWarrantAction") {
                emsApp.changePage("Warrants");
            } else if (event.data.type == "completedBoloAction") {
                emsApp.changePage("Bolos");
            } else if (event.data.type == "returnedReportDetails") {
                emsApp.changePage("Search Reports");
                emsApp.report_selected = event.data.details;
            } else if (event.data.type == "recentReportsAndWarrantsLoaded") {
                emsApp.homepage.reports = event.data.reports;
                emsApp.homepage.warrants = event.data.warrants;
                emsApp.homepage.bolos = event.data.bolos;
                emsApp.officer.name = event.data.officer;
            };
        });
    };
};
...
document.onkeydown = function (data) {
    if (data.which == 27 || data.which == 112) { // ESC or F1
        ...
        // Stop enter key from crashing ems in an input?
...
function WarrantTimer() {
    setInterval(function() {
        for (let key in emsApp.warrants) {
            const warrant = emsApp.warrants[key];
            ...
            if (t >= 0) {
                ...
                Vue.delete(emsApp.warrants, key)
            }
        }
    }, 1000);
}
...
function BoloTimer() {
    setInterval(function() {
        for (let key in emsApp.bolos) {
            const bolo = emsApp.bolos[key];
