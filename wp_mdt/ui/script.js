    ...
    methods: {
        changePage(page) {
            ...
            switch (page) {
                ...
                case "Warrants": {
                    $.post("http://wp_mdt/getWarrants", JSON.stringify({}));
                    $("#warrants").addClass("nav-active");
                    break;
                }
                case "Bolos": {
                    $.post("http://wp_mdt/getBolos", JSON.stringify({}));
        ...
        closeMDT() {
            $.post("http://wp_mdt/close", JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {
                this.offender_results.query = this.offender_search;

                $.post("http://wp_mdt/performOffenderSearch", JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (let key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    $.post("http://wp_mdt/viewOffender", JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post("http://wp_mdt/saveOffenderChanges", JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post("http://wp_mdt/performReportSearch", JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && this.report_new.incident) {
                $.post("http://wp_mdt/submitNewReport", JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post("http://wp_mdt/getOffender", JSON.stringify({
                char_id: id
            }));
        },
        ToggleReportEdit() {
        ...
        DeleteSelectedReport() {
            $.post("http://wp_mdt/deleteReport", JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post("http://wp_mdt/saveReportChanges", JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post("http://wp_mdt/vehicleSearch", JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post("http://wp_mdt/getVehicle", JSON.stringify({
                vehicle: result
            }));
        },
        WarrantReportSearch() {
        ...
        SubmitNewWarrant() {
            ...
            $.post("http://wp_mdt/submitNewWarrant", JSON.stringify({
        ...
        SubmitNewBolo() {
            ...
            $.post("http://wp_mdt/submitNewBolo", JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post("http://wp_mdt/deleteWarrant", JSON.stringify({
        ...
        DeleteSelectedBolo() {
            $.post("http://wp_mdt/deleteBolo", JSON.stringify({
        ...
        OpenReportById(id) {
            $.post("http://wp_mdt/getReport", JSON.stringify({
                id: id
            }));
        },
        ...
        SentencePlayer() {
            $.post("http://wp_mdt/sentencePlayer", JSON.stringify({
                id: this.report_selected.id,
    ...
    if (document.readyState === "complete") {
        window.addEventListener("message", function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.officer.job = event.data.job;
                mdtApp.officer.grade = event.data.grade;
            };
        });
    };
};
