...
function CreateOxyPed()
	...
	deliveryPed = CreatePed(pedType, hashKey, OxyDropOffs[rnd]["x"],OxyDropOffs[rnd]["y"],OxyDropOffs[rnd]["z"], OxyDropOffs[rnd]["h"], true)
    ...
    while true do
		...
		if OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				exports['mythic_notify']:SendAlert('inform', 'Car is to messed up, dealer bailed.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' }) -- RED
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
