...
local inHouse = false
local alreadylockpicking = false
...
Citizen.CreateThread(function()
    ...
    while true do  
        if isNight() then
            ...
            if not inHouse and not alreadylockpicking then
                for id,v in pairs(robbableHouses) do
                    if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 1.0 then
                        while GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 1.0 do
                            playerCoords = GetEntityCoords(ped, true)
                            if tableContains(openhouse, id) then
                                DrawText3Ds( v.x, v.y, v.z, '~w~Press ~g~H~s~ To enter house.')
                                if IsControlJustPressed(0, 74) then 
                                    TriggerEvent('houseRobberies:enterhouse', id)
                                end
                            else
                                DrawText3Ds( v.x, v.y, v.z, '~w~Press ~g~H~s~ To Attempt lockpick')
                                if IsControlJustPressed(0, 74) then
                                    if not alreadylockpicking then
                                        alreadylockpicking = true
                                        TriggerServerEvent('houseRobberies:NewPick')
                                    else
                                        exports['mythic_notify']:SendAlert('inform', 'You are already breaking in!', 2500, { ['background-color'] = '#ff0000', ['color'] = '#000000' })
                                    end
                                end
                            end
                            Wait(0)
...
Citizen.CreateThread(function()
    while true do
        if isRobbing and DoesEntityExist(safe) and not robbedsafe then
            local ped = PlayerPedId()
            local playerCoords = GetEntityCoords(ped, true)
            if #(playerCoords-safepos) <= 3.0 then
                while #(playerCoords-safepos) <= 3.0 do
                    playerCoords = GetEntityCoords(ped, true)
                    DrawText3Ds( safepos.x, safepos.y, safepos.z, '~w~Press ~g~G~s~ Attempt safe cracking')
                    if IsControlJustPressed(0, 47) and not robbedsafe then
                        robbedsafe = true
...
AddEventHandler('houseRobberies:attempt', function(lockpicks)
 if lockpicks >= 1 and isNight() and not isRobbing then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  local streetName,_ = GetStreetNameAtCoord(playerCoords[1], playerCoords[2], playerCoords[3])
  local streetName = GetStreetNameFromHashKey(streetName)
  local zone = tostring(GetNameOfZone(playerCoords[1], playerCoords[2], playerCoords[3]))
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    exports["t0sic_loadingbar"]:StartDelayedFunction("Lockpicking Property", 12000, function()
     pedSpawned = false
     isLockpicking = false
     alreadylockpicking = false
     --if math.random(1, 10) >= 1 then
     TriggerServerEvent("houseRobberies:log")
     if math.random(1, 10) >= 3 then --70% chance to call cops
      exports['wp_dispatch']:addCall("10-31", "Breaking And Entering", {
        {icon="fas fa-road", info = streetName ..", ".. area}
        }, {playerCoords[1], playerCoords[2], playerCoords[3]}, "police", 3000, 40, 1 )
        exports['wp_dispatch']:addCall("10-31", "Breaking And Entering", {
            {icon="fas fa-road", info = streetName ..", ".. area}
            }, {playerCoords[1], playerCoords[2], playerCoords[3]}, "dispatch", 3000, 40, 1 )
     end
     ...
     TriggerServerEvent('houseRobberies:syncserver', id)
    end)
   end
  end
 elseif lockpicks == 0 and not isRobbing then
  exports['mythic_notify']:SendAlert('inform', 'You need a lockpick!', 2500, { ['background-color'] = '#ff0000', ['color'] = '#000000' })
  isLockpicking = false
  alreadylockpicking = false
 end
end)

RegisterNetEvent('houseRobberies:syncclient')
AddEventHandler('houseRobberies:syncclient', function(id)
    table.insert( openhouse, id)
    table.insert(deleteHouses, id)
end)



RegisterNetEvent('houseRobberies:enterhouse')
AddEventHandler('houseRobberies:enterhouse', function(id)
    ...
    if tableContains(deleteHouses, id) then
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(PlayerPedId())) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press ~g~E~s~ To Search ' .. myRobbableItems[i]["name"])
     ...
     if IsControlJustReleased(0, 38) then
      ...
      exports["t0sic_loadingbar"]:StartDelayedFunction("Searching "..myRobbableItems[i]['name'], 20000, function()
       ...
       TriggerServerEvent('houseRobberies:LogItem')
      end)
     end
    end
 	 end
...
function agroNPC()
 ...
 repeat
  ...
  if distance < 35.0 and ped ~= PlayerPedId() and not IsPedAPlayer(ped) then
   ...
   GiveWeaponToPed(ped, GetHashKey('weapon_unarmed'), 150, 0, 1)
...
function CreateSafe(x,y,z)
	safespawned = true
	safepos = vector3(x, y, z)--{["x"] = x, ["y"] = y, ["z"] = z}
...
AddEventHandler('safe:success', function()
 ...
 safepos = vector3(0, 0, 0)--{["x"] = 0, ["y"] = 0, ["z"] = 0}
 TriggerServerEvent("houseRobberies:LogSafe")
 TriggerServerEvent('houseRobberies:giveMoney')
 TriggerServerEvent('houseRobberies:LogMoney')
end)
