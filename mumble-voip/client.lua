...
RegisterCommand("radiovolume", function(source, args, rawCommand)
local message = table.concat(args, '')

if message == "1.0" then
	natti = 1.0
elseif message == "0.9" then
	natti = 0.9
elseif message == "0.8" then
	natti = 0.8
elseif message == "0.7" then
	natti = 0.7
elseif message == "0.6" then
	natti = 0.6
elseif message == "0.5" then
	natti = 0.5
elseif message == "0.4" then
	natti = 0.4
elseif message == "0.3" then
	natti = 0.3
elseif message == "0.2" then
	natti = 0.2
elseif message == "0.1" then 

	print(message)
	natti = 0.1
else
	print("Must contain the following: 1.0/0.9/0.8/0.7/0.6/0.5/0.4/0.3/0.2/0.1")
	TriggerEvent('chat:addMessage', {
	color = { 255, 0, 0},
	multiline = true,
	args = {"Odessa Radio: ", "Must contain the following: 1.0/0.9/0.8/0.7/0.6/0.5/0.4/0.3/0.2/0.1   Default is 0.4"}
	})
end
end, false)
...
function TogglePlayerVoice(serverId, value)
	...
	if value then
		if not unmutedPlayers[serverId] then
			...
			MumbleSetVolumeOverrideByServerId(serverId, natti)
			msg = true
		end
	else
		...
		if initialised then
			...
			if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
				...
				if not secondaryPressed then
                    ...
                    exports['wp_hud']:setVoiceRange(voiceMode * 35)
