...
AddEventHandler('esx_property:hasEnteredMarker', function(name, part, parking)
        ...
        if part == 'ExteriorEntryPoint' then
            if PlayerData.job.name == 'police' then
				...
				text = "This garage is for POLICE ONLY!",
				type = "error",
				timeout = (5000),
				layout = "centerLeft",
				queue = "global"
	...
	if part == 'Parking' then
		...
		if IsPedInAnyVehicle(playerPed,  false) and not IsAnyVehicleNearPoint(parkingPos.x,  parkingPos.y,  parkingPos.z,  1.0) then
			...
			TriggerServerEvent('wp_policeGarage:setParking', name, parking, vehicleProps)

			if Config.EnableOwnedVehicles then
				TriggerServerEvent('wp_policeGarage:updateOwnedVehicle', vehicleProps)
			end
...
AddEventHandler('esx_property:hasExitedMarker', function(name, part, parking)
	...
	if part == 'Parking' then
		...
		if IsPedInAnyVehicle(playerPed,  false) and not IsAnyVehicleNearPoint(parkingPos.x,  parkingPos.y,  parkingPos.z,  1.0) then
			TriggerServerEvent('wp_policeGarage:setParking', name, parking, false)
		end
