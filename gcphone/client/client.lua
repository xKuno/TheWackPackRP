...
function NoPhone() 
  ...
  exports['mythic_notify']:SendAlert('inform', 'You do not have a phone.', 2500, { ['background-color'] = '#b50000', ['color'] = '#ffffff' })
end
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    exports['mythic_notify']:SendAlert('inform', ''..text, 2500, { ['background-color'] = '#00b51e', ['color'] = '#ffffff' })
...
--====================================================================================
function sendMessage(num, message, coords)
  TriggerServerEvent('gcPhone:sendMessage', num, message, coords)
end
...
AddEventHandler('gcphone:autoCall', function(number, extraData)
  ...
  if number == '911' then
    exports['wp_dispatch']:addCall("9-1-1", "Incomming 9-1-1 At MRPD", {
        {icon = "fas fa-road", info = 'Sinner Street, Mission Row'}
    }, {441.29, -996.61, 34.97}, "police", 3000, 162, 1)
    exports['wp_dispatch']:addCall("9-1-1", "Incomming 9-1-1 At MRPD", {
      {icon = "fas fa-road", info = 'Sinner Street, Mission Row'}
    }, {441.29, -996.61, 34.97}, "dispatch", 3000, 162, 1)
  elseif number == '311' then
    exports['wp_dispatch']:addCall("3-1-1", "Incomming 3-1-1 At Pillbox", {
      {icon = "fas fa-road", info = 'Elgin Ave, Pillbox Hill'}
    }, {311.12, -597.23, 43.28}, "ambulance", 3000, 162, 1)
  end
end)
...
RegisterNUICallback('sendMessage', function(data, cb)
  ...
  if data.phoneNumber == 'police' then

    local coords = GetEntityCoords(PlayerPedId())
    exports['wp_dispatch']:addMessage(data.message, {coords[1], coords[2], coords[3]}, 'police', 10000, 162, 1 )
    exports['wp_dispatch']:addMessage(data.message, {coords[1], coords[2], coords[3]}, 'dispatch', 10000, 162, 1 )

  elseif data.phoneNumber == 'ambulance' then

    local coords = GetEntityCoords(PlayerPedId())
    exports['wp_dispatch']:addMessage(data.message, {coords[1], coords[2], coords[3]}, 'ambulance', 10000, 162, 1 )
  end
end)
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
      exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/822920022358753290/uadlaEtP7TGIcttXaFNmoOpdVvS1KRD36zBzGkwWVIEBmWb46s882aFv4ZiArhGMgB5N", data.field, function(data)
  ...
  RegisterNetEvent('vulcan_prison:sendToLawText')
  AddEventHandler('vulcan_prison:sendToLawText', function(num, message)
    local coords = vector3(1828.85, 2580.38, 46.01)
    sendMessage(num, message, coords)
  end)