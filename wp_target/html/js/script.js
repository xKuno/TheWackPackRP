    ...
    if (item.response == 'openTarget') {
        ...
        $.each(item.data, function (index, item) {
            ...
            $("#target-"+index).hover((e)=> {
                $("#target-"+index).css("color",e.type === "mouseenter"?"rgb(229, 186, 7)":"white")
            })
        ...
        $(".target-eye").css("color", "rgb(229, 186, 7)");
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('http://wp_target/selectTarget', JSON.stringify({
            event: TargetData,
        }));
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $(".target-label").html("");
            $('.target-wrapper').hide();
            $.post('http://wp_target/closeTarget');
            break;
    }
});
