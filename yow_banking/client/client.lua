...
-- Citizen.CreateThread(function()
-- 	local playerlocatm = GetEntityCoords(PlayerPedId())

-- 	for _, searchatm in pairs2(atms) do
-- 		local distance = GetDistanceBetweenCoords(searchatm.x, searchatm.y, searchatm.z, playerlocatm['x'], playerlocatm['y'], playerlocatm['z'], true)
		
-- 		if distance <= 1 then
-- 			if (GetDistanceBetweenCoords(pos, searchatm.x ,  searchatm.y , searchatm.z, true) < 3) then
-- 				DrawMarker(26, searchatm.x, searchatm.y, searchatm.z-0.96, 0, 0, 0, 0, 0, 0, 1.20, 1.20, 1.20, 34, 39, 48, 200, 0, 0, 0, 0)
-- 				DrawText3D(searchatm.x, searchatm.y, searchatm.z + 0.2, "~b~[E]~w~Open Bank")
...
--==             Core Threading                ==
--===============================================
RegisterNetEvent('yow_banking:showATM')
AddEventHandler('yow_banking:showATM', function()
	inMenu = true
...
RegisterNetEvent('yow_banking:ATMCheck')
AddEventHandler('yow_banking:ATMCheck', function()
    ...
    if atmFound == true then
		...
		exports['mythic_notify']:SendAlert('error', 'No atm is here')
...
RegisterNUICallback('login', function(data)
	...
	TriggerServerEvent('yow_banking:getAccount', data.login)
end)
...
RegisterNUICallback('loginbus', function(data)
	...
	if job and jobname == Config.Police or job and jobname == Config.Mayor then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	elseif job and jobname == Config.EMS or job and jobname == Config.Mayor then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	elseif job and jobname == Config.Cardealer then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	elseif job and jobname == Config.DOJ or job and jobname == Config.DOJ2 or job and jobname == Config.DOJ3 or job and jobname == Config.Mayor then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	elseif job and jobname == Config.WN then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	elseif job and jobname == Config.Mayor then
		TriggerServerEvent('bank:loginbus', data.login, data.password)
		TriggerServerEvent('yow_banking:getAccount', data.login)

	else

		exports['mythic_notify']:SendAlert('error', 'You Can\'t login to this account.')
...
-- RegisterNUICallback('jobbank', function(data)

-- 	print("lol working")
-- 	local xPlayer = ESX.GetPlayerData()
-- 	local job = xPlayer.job
-- 	local jobname = xPlayer.job.name
-- 	if job and jobname == 'police' and xPlayer.job.grade_name == 'boss' then
-- 		SendNUIMessage({
-- 			type = "succlogin"
-- 		})
-- 		TriggerServerEvent('bank:login', yowpoliceboyman, yowpolicemymam)
-- 		TriggerServerEvent('yow_banking:getAccount', yowpoliceboyman)

-- 	-- elseif job and jobname == 'ambulance' and xPlayer.job.grade_name == 'boss' then
-- 	-- 	TriggerServerEvent('bank:login', yowpoliceboyman, yowpolicemymam)
-- 	-- 	TriggerServerEvent('yow_banking:getAccount', yowpoliceboyman)
...
RegisterNUICallback('createaccountnew', function(data)
	TriggerServerEvent('yow_banking:createnewaccount', data.login12, data.password1, data.pin)
end)



RegisterNUICallback('changepassword', function(data)
	TriggerServerEvent('yow_banking:login', data.login, data.password)
end)
...
RegisterNUICallback('transfer', function(data)
	TriggerServerEvent('bank:transfer2', data.to, data.amountt)
end)
...
RegisterNUICallback('NUIFocusOff', function()
	TriggerServerEvent('yow_banking:updateLoggedFalse')
end)
