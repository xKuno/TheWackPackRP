...
AddEventHandler("RollDice:Client:Roll", function(sourceId, maxDinstance, rollTable, sides, location)
    for _,d in ipairs(diceRoll) do
        if #(vector3(d[1],d[2],d[3]) - GetEntityCoords(PlayerPedId())) < 65 then
	...
	if GetPlayerServerId(PlayerId()) == sourceId then --Checks if you you ahve the same source id
        ...
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3, 'dice', 0.5)
    else
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', length = 2500, text = 'You can not roll here.'})
        end
    end
end)
