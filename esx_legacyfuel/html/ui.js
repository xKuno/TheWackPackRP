$(document).ready(function(){
      ...
      $(document).keyup(function(e) {
        if (!counting) {
          if (e.key === "Escape") {
            ...
            completar;
            maxFuel;
            $.post('http://esx_legacyfuel/escape', JSON.stringify({}));
          }
        }
      });
    } else {
  ...
  function increase() {
    if (counting) {
      if (fuel < maxFuel) {
        ...
        $.post('http://esx_legacyfuel/removeanim', JSON.stringify({}));
        if (completar) {
          $.post('http://esx_legacyfuel/pay', JSON.stringify({ new_perc: perc_new*dataGas.price }));
          perc_new;
          completar = !completar;
        } else if (guardar) {
          $.post('http://esx_legacyfuel/pay', JSON.stringify({ new_perc: price }));
          price;
          guardar = !guardar;
        }
        counting = !counting;
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      $.post('http://esx_legacyfuel/checkpay', JSON.stringify({ new_perc: perc_new }));
      counting = !counting;
      completar = !completar;
      maxFuel = 99
      $('.modal span').text("Fill up for $ "+(perc_new*dataGas.price).toFixed(2));  
      $('.modal').fadeIn(300);
      $('.container').fadeOut();
      $('left').fadeOut();
      $('.act').fadeOut();
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          $.post('http://esx_legacyfuel/checkpay', JSON.stringify({ new_perc: price }));
          counting = !counting;
          guardar = !guardar;
          maxFuel = (Math.floor(price/dataGas.price)+Math.round(datafuel))-1
          $('.modal span').text("Fill up for $ "+price); 
          $('.modal').fadeIn(300);
          $('.container').fadeOut();
          $('left').fadeOut();
          $('.act').fadeOut();
        } else {
          var texto = "The price to fill the tank is <b>$"+((100-Math.round(datafuel))*dataGas.price)+"</b>!"
          $.post('http://esx_legacyfuel/notifytext', JSON.stringify({ text: texto }));
        }
      } else {
        var texto = "The amount must be higher than $" + dataGas.price + "!"
        $.post('http://esx_legacyfuel/notifytext', JSON.stringify({ text: texto }));
      }
    }
  });
  ...
  $(".accept").click(function() {
    if (counting) {
      ...
      $('left').fadeIn(300);
      $('.act').fadeIn(300);
      $.post('http://esx_legacyfuel/startanim', JSON.stringify({}));
      myStop();
      myStart(); 
    }
  })
