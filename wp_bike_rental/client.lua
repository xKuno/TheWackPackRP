...
function OpenBikesMenu()
	...
	if Config.EnablePrice == false then
		...
		table.insert(elements, {label = _U('bike4free'), value = 'kolo5'}) 
	end
	...
	if data.current.value == 'kolo5' then
		...
		if Config.EnablePrice then
			TriggerServerEvent("esx:lowmoney", Config.PriceSkateboard) 
		end
		...
		if Config.EnableEffects then
			...
			DoScreenFadeIn(3000) 
			ESX.ShowNotification(_U('bike_pay', Config.PriceSkateboard))
		else
			TriggerEvent('esx:spawnVehicle', "skateboard")
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.PriceSkateboard))
		end
		ESX.UI.Menu.CloseAll()
		havebike = true	
	end
