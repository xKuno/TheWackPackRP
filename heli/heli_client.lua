...
-- Config
local fov_max = 90.0
local fov_min = 2.5 -- max zoom level (smaller fov is more zoom)
...
local toggle_vision = 25 -- control id to toggle vision mode. Default: Right mouse btn
local toggle_spotlight = 74 -- control id to toggle the various spotlight states Default: INPUT_PhoneCameraGrid (G)
local toggle_display = 44 -- control id to toggle vehicle info display. Default: INPUT_COVER (Q)
local lightup_key = 172 -- control id to increase spotlight brightness. Default: INPUT_MP_TEXT_CHAT_TEAM (UP-Arrow)
local lightdown_key = 173 -- control id to decrease spotlight brightness. Default: INPUT_CELLPHONE_DOWN  (DOWN-Arrow)
local radiusup_key = 175 -- control id to increase manual spotlight radius. Default: INPUT_VEH_PUSHBIKE_SPRINT (RIGHT-Arrow)
local radiusdown_key = 174 -- control id to decrease spotlight radius. Default: INPUT_SPRINT (LEFT-Arrow)
local maxtargetdistance = 1000 -- max distance at which target lock is maintained
local brightness = 10.0 -- default spotlight brightness
local spotradius = 8.0 -- default manual spotlight radius
...
local manual_spotlight = true
local tracking spotlight = true
...
local polmav_hash2 = GetHashKey("seasparrow")
local polmav_hash3 = GetHashKey("polmav")
local polmav_hash4 = GetHashKey("emsaw139")
local polmav_hash5 = GetHashKey("polas350")
local polmav_hash6 = GetHashKey("whitehawk")
local fov = (fov_max+fov_min)*0.7
	...
	while true do
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				if IsDisabledControlJustPressed(0, toggle_display) then 
					ChangeDisplay()
				end
...
function IsPlayerInPolmav()
	...
	return IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, polmav_hash2) or IsVehicleModel(vehicle, polmav_hash3) or IsVehicleModel(vehicle, polmav_hash4) or IsVehicleModel(vehicle, polmav_hash5) or IsVehicleModel(vehicle, polmav_hash6)
end
